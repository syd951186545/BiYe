...
def best_child(node, is_exploration):  # 若子节点都扩展完了，求UCB值最大的子节点
    ...
    for sub_node in node.get_children():
        ...
        # UCT--
        # left = sub_node.get_quality_value() / sub_node.get_visit_times()
        # right = node.get_visit_times()/ sub_node.get_visit_times()
        # score = left + C *right
...
class State(object):  # 某游戏的状态，例如模拟一个数相加等于1的游戏
    ...
    def compute_reward(self):
        ...
        return 1 if self.node_parent == self.target_node or self.current_node == self.target_node else 0
...
if __name__ == '__main__':
    ...
    state_0.set_cumulative_choices([target_query, start_node])
